...
import { reloadTime } from "@/assets/js/constant";
 ...
 * 次の24時までの時間を取得
 */
export const diffToNextEndOfDate = () => {
    const now = dayjs(new Date()).locale("ja");
    const reloadTimeObject = now
        .hour(reloadTime.hour)
        .minute(reloadTime.minute)
        .second(reloadTime.second)
        .millisecond(reloadTime.millisecond);
    if (reloadTimeObject.diff(now) > 0) {
        return reloadTimeObject.diff(now);
    }

    const nextReloadTimeObject = reloadTimeObject.add(1, "day");
    return nextReloadTimeObject.diff(now);
};
