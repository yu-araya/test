...
(function()
{
	...
	function showPageCount(count)
	{
		...
		changeHTML("count", maxCount + "件（" + page + "/" + maxPage + "）");
	...
	function showPage(rows)
	{
		...
		for (var i = 0; i < TABLE_ROW; i++)
		{
			...
			else
			{
				...
				changeHTML("r" + i + "3",statusList[rows.item(i).status]);
			}
		}
	...
	function saveLog()
	{
		database.readTransaction(function (tx)
		{
			...
			function (tx,rs)
			{
				...
				// 書き込みデータ
				param.data = "時刻,ID,社員コード,社員区分,食事区分,ステータス" + '\r\n';
				...
				for (var i = 0; i < rs.rows.length; i++) {
					...
					param.data += row.employeeId + ',';
					param.data += row.employeeKbn + ',';
					param.data += row.fno + ',';
